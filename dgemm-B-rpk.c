...
static void do_block(int lda, int k, int n, double* A, double* B, double* C, double* A_old) {
    ...
    for (int o = 0; o < n;  o++) {
        ...
        for (int m= 0; m < k; m++) {
            ...
            A_col_1 = _mm256_loadu_pd(&A[m_r*m]);
            A_col_2 = _mm256_loadu_pd(&A[m_r*m + 4]);
...
static void do_naive(int n, double* A, double* B, double* C) {
    // For each row i of A
    for (int i = 0; i < m_r; ++i) {
...
void square_dgemm(int lda, double* A, double* B, double* C) {
    ...
    for (int j = 0; j < total; j++) {
        ...
        double* A_rpk = (double *) _mm_malloc(m_c * k_c * sizeof(double), 64);
        ...
        for (int k = 0; k < total; k += k_c) {
            // Accumulate block dgemms into block of C   
             ...
             for (int i = 0; i < total; i += m_c) {
                ...
                //Now, we can pack these macro-kernels of A for contiguous access

                int n_row = m_c;
                int n_col = k_c;
                int n_rows_p_block = m_r;
                int W = n_row * n_col;

                //Now, we ought to loop over the entries in A and find the corresopnding repacked indices
                for (int t = 0; t < W; t++) {

                    //First, we find the a_ij indices if this matrix were not column ordered
                    int index_i = floor(t/n_row) + 1;
                    int index_j = t % n_col + 1;
                   ...
                   // A_rpk[index] = A_b[k];
                    A_rpk[index] = A_b[(index_i-1) + (index_j-1)*total];
                ...
                for (int b = 0; b < B_bord; b += n_r) {
                    for (int c = 0; c < C_bord; c += k_r) {
                       for (int a = 0; a < A_bord; a += m_r) {
                           ...
                           do_block(total, k_r, n_r, A_rpk_2, B_b_2, C_b_2, A_b_2);
                      }
                   }
                }
            }
